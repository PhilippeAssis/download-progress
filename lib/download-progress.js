...
DownloadProgress.prototype.get = function(callbackAllFilesFetched) {
    ...
    async.eachSeries(self.urls, function(url, callbackRequestDone) {
        self._getFile(url.url, url.dest, url.displayMessage, function(err) {
            if (err) {
                throw new Error(err);
            }
            callbackRequestDone();
...
DownloadProgress.prototype._getFile = function(srcUrl, copyPath, displayMessage, callback) {
    ...
    req.on('response', function(res) {
        ...
        options = {
            ...
            displayMessage: (self.options.displayMessage == false) ? false : true, // Legacy will not be used anymore
            progress:{
        ...
        if (options.displayMessage !== false) {
            // Modification by Toyz@Github https://github.com/Toyz
            // Allows you to set a displayMessage for each item in the URLs list now
            console.log(displayMessage || srcUrl.href);
        }
